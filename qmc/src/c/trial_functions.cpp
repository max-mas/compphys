...
double harmonic_local_erg_2d_2p(std::vector<double> alpha_x) { //TODO optimise function calls
    ...
    double kinetic_term_x1 = -0.5 * harmonic_trial(alpha_x) *
             (-1 + lambda/(s * pow(1 + alpha * s, 2))
             - lambda * pow(x1 - x2, 2) / (pow(s, 3) * pow(1 + alpha * s, 3)) * (1 + 3 * alpha * s)
             + pow(-x1 + lambda * (x1 - x2) / (s * pow(1 + alpha * s, 2)), 2) );
    double kinetic_term_x2 = -0.5 * harmonic_trial(alpha_x) *
             (-1 + lambda/(s * pow(1 + alpha * s, 2))
              - lambda * pow(x2 - x1, 2) / (pow(s, 3) * pow(1 + alpha * s, 3)) * (1 + 3 * alpha * s)
              + pow(-x2 + lambda * (x2 - x1) / (s * pow(1 + alpha * s, 2)), 2) );
    double kinetic_term_y1 = -0.5 * harmonic_trial(alpha_x) *
             (-1 + lambda/(s * pow(1 + alpha * s, 2))
              - lambda * pow(y1 - y2, 2) / (pow(s, 3) * pow(1 + alpha * s, 3)) * (1 + 3 * alpha * s)
              + pow(-y1 + lambda * (y1 - y2) / (s * pow(1 + alpha * s, 2)), 2) );
    double kinetic_term_y2 = -0.5 * harmonic_trial(alpha_x) *
             (-1 + lambda/(s * pow(1 + alpha * s, 2))
              - lambda * pow(y2 - y1, 2) / (pow(s, 3) * pow(1 + alpha * s, 3)) * (1 + 3 * alpha * s)
              + pow(-y2 + lambda * (y2 - y1) / (s * pow(1 + alpha * s, 2)), 2) );
